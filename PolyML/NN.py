...
# assume only one non-SMILES column is target
...
# 3) Feature functions
# ======================
def compute_rdkit_descriptors_safe(smiles: str):
    """Compute a small set of RDKit descriptors; return None on failure."""
    try:
        ...
        desc = {
            ...
            "NumHeteroatoms": sum(
                1 for a in mol.GetAtoms() if a.GetSymbol() not in ("C", "H")
            ),
...
def compute_smiles_features(smiles: str):
    """Simple string-based SMILES heuristics when RDKit isn't available."""
    s = str(smiles)
...
def featurize_list(smiles_list):
    """Featurize a list of SMILES strings using RDKit or fallback heuristics."""
    feats = []
    if use_rdkit:
        rdkit_keys = [
            "MolWt",
        ...
        for smi in smiles_list:
            ...
            if d is None:
                d = {k: np.nan for k in rdkit_keys}
            feats.append(d)
    else:
        for smi in smiles_list:
...
# ======================
# 8) Cross-validation: R2, RMSE, MAE
# ======================
kf = KFold(n_splits=5, shuffle=True, random_state=42)

r2s = cross_val_score(pipe, X, y, scoring="r2", cv=kf, n_jobs=-1)
rmses = np.sqrt(
    -cross_val_score(pipe, X, y, scoring="neg_mean_squared_error", cv=kf, n_jobs=-1)
)
...
# ======================
# 9) Fit full model on all training data
# ======================
pipe.fit(X, y)
...
res = permutation_importance(
    pipe, X, y, n_repeats=8, random_state=42, n_jobs=-1
)
...
for name, val in perm_sorted[:10]:
    print(name, val)

# ======================
# 11) Prepare test features and predict
# ======================
...
# ======================
# 13) Simple interpretability plot: top feature vs target
# ======================
...
try:
    ...
    X_scaled = scaler.transform(train_feats_valid[selected_feature_names])
    X_trans = sel.transform(X_scaled)

    explainer = shap.TreeExplainer(rf)
    shap_vals = explainer.shap_values(X_trans)

    shap.summary_plot(
        shap_vals, X_trans, feature_names=selected_feature_names, show=False
    )
    plt.tight_layout()
